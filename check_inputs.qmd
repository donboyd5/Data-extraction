...
HH <- here::here()
newfile <- paste0(fs::path_ext_remove(template), "_check.xlsx")
...
get_data <- function(sheet, path = check_path) {
  ...
  range <- NULL
  if ("range" %in% info$item) {
    range <- info |> filter(item == "range") |> pull(value)
  }
...
sheets_to_show <- sheets |> stringr::str_subset("headcount")
show_sheets(sheets_to_show)
...
sheets_to_show <- sheets |>
  stringr::str_subset("salary") |>
  stringr::str_subset("growth", negate = TRUE)
...
sheets_to_show <- sheets |>
  stringr::str_subset("salary_growth")
